...
def flagged_cells(hd_export, py_input, hd_row, py_row, flag_num, skipped):
...
def main(hd_export, py_input):
    ...
    for n in range(0, len(hd_export)):
        ...
        if py_input[p]['Out'] == '':
            ...
            flagged_cells(hd_export, py_input, n, p, flag_count, skipped=False)
            flag_count += 1
        ...
        if not start_time - timedelta(minutes=10) <= time_in <= start_time + timedelta(minutes=15):
            # updates the flagged_items sheet with info about that shift
            flagged_cells(hd_export, py_input, n, p, flag_count, skipped=True)
            flag_count += 1
